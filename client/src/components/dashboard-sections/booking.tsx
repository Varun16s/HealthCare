...
export default function BookingSection() {
  ...
  const fillRecommendedTime = (time: string) => {
    const timeValue = time.includes("AM") ? 
      time.replace(" AM", "").padStart(5, "0") :
      (parseInt(time.split(":")[0]) + 12).toString().padStart(2, "0") + ":" + (time.split(":")[1]?.replace(" PM", "") || "00");
  ...
  return (
    <div className="p-8" data-testid="booking-section">
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <Select onValueChange={field.onChange} value={field.value}>
                    <FormControl>
                      <SelectTrigger data-testid="select-booking-support-type">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <Select onValueChange={field.onChange} value={field.value}>
                    <FormControl>
                      <SelectTrigger data-testid="select-booking-specialist">
                ...
                <FormControl>
                  ...
                  >
                    <div className="flex items-center space-x-2">
                      <RadioGroupItem value="video" id="booking-video" data-testid="radio-booking-video" />
                      <Label htmlFor="booking-video">Video call</Label>
                    </div>
                    <div className="flex items-center space-x-2">
                      <RadioGroupItem value="phone" id="booking-phone" data-testid="radio-booking-phone" />
                      <Label htmlFor="booking-phone">Phone call</Label>
                    </div>
                    <div className="flex items-center space-x-2">
                      <RadioGroupItem value="chat" id="booking-chat" data-testid="radio-booking-chat" />
                      <Label htmlFor="booking-chat">Text chat</Label>
                    </div>
                    <div className="flex items-center space-x-2">
                      <RadioGroupItem value="in-person" id="booking-in-person" data-testid="radio-booking-in-person" />
                      <Label htmlFor="booking-in-person">In-person</Label>
                    </div>
                  </RadioGroup>
                </FormControl>
                <FormMessage />
          ...
          <CardContent className="p-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h4 className="font-medium text-sage-700" data-testid="recommendation-name">
                  {recommendation.name}
                </h4>
                <span className="text-sm bg-green-100 text-green-700 px-2 py-1 rounded-full" data-testid="recommendation-rating">
                  {recommendation.rating} rating
                </span>
              </div>
              <p className="text-sage-600" data-testid="recommendation-details">
                ...
                <div className="flex flex-wrap gap-2">
                  {recommendation.availability.map((time) => (
                    <Button
                      ...
                      variant="outline"
                      size="sm"
                      onClick={() => fillRecommendedTime(time)}
                      className="bg-primary-100 text-primary-700 border-primary-200 hover:bg-primary-200"
                      data-testid={`button-recommended-time-${time.replace(/[^a-z0-9]/gi, '-').toLowerCase()}`}
                    >
                      {time}
                    </Button>
                  ))}
