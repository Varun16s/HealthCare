...
export default function AIChat() {
  ...
  return (
    <div className="h-full" data-testid="ai-chat">
      {/* Header */}
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {quickMessages.map((message) => (
            <Button
              ...
              disabled={sendMessageMutation.isPending}
              className="text-sm bg-sage-100 text-sage-700 hover:bg-sage-200"
              data-testid={`button-quick-${message.toLowerCase().replace(/\s+/g, '-')}`}
            >
              {message}
            </Button>
          ))}
